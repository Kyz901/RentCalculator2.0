...
public class UserRepository {
    ...
    public UserRepository(
        final NamedParameterJdbcOperations operations, final MultiLogger log
    ) {
    ...
    public User fetchUserByLogin(final String login) {
        ...
        if (users.isEmpty()) {
            ...
            log.info(() -> String.format("IN [fetchUserByLogin]: - no user was found for login: %s", login));

            return null;
        } else if (users.size() > 1) {
            // Handle the case when multiple users are found
            log.error(() -> String.format("IN [fetchUserByLogin]: - Multiple users found for login: %s", login));
